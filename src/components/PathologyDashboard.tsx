...
export default function PathologyDashboard({profile,user,activeTab,onTabChange,uiPrefix,subDepartmentId}:{profile:any;user:any;activeTab?:string;onTabChange?:(t:string)=>void;uiPrefix?:string;subDepartmentId?:string}){
  const[tab,setTab]=useState(activeTab||"overview");
  useEffect(()=>{if(activeTab)setTab(activeTab);},[activeTab]);
  const sw=(t:string)=>{setTab(t);onTabChange?.(t);};
  const withSub=(url:string)=>{if(!subDepartmentId)return url;return url.includes("?")?`${url}&subDepartmentId=${encodeURIComponent(subDepartmentId)}`:`${url}?subDepartmentId=${encodeURIComponent(subDepartmentId)}`;};
  const withSubBody=(body:any)=>{if(!subDepartmentId||!body||typeof body!=="object")return body;return{...body,subDepartmentId:body.subDepartmentId||subDepartmentId};};
  ...
  const createOrderFromWizard=async()=>{if(!refWiz||!wizItems.length)return;setWizSaving(true);try{const res=await fetch(withSub("/api/pathology/orders"),{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({patientId:refWiz.patientId,priority:wizPriority,orderType:"DOCTOR_REFERRAL",clinicalNotes:wizNotes,referralNotes:refWiz.referralNote,appointmentId:refWiz.appointmentId,items:wizItems,billingAction:wizBillingAction,paymentMethod:wizPayMethod}))}).then(r=>r.json());if(res.success){const billCreated=res.data?.bill;setRefWiz(null);if(billCreated&&wizBillingAction==="send_to_billing")alert(`✅ Lab Order created. Bill #${billCreated.billNo} sent to Billing/Reception.`);await Promise.all([ldStats(),ldOrders()]);}else alert(res.message||"Failed to create order");}catch(e:any){alert("Network error: "+(e.message||"Please try again"));}finally{setWizSaving(false);};};

  const ldStats=useCallback(async()=>{setSL(true);const d=await fetch(withSub("/api/pathology/stats"),{credentials:"include"}).then(r=>r.json());if(d.success){setStats(d.data);setReferralQueue(d.data.referralQueue||[]);if(d.data.hospital)setHospitalInfo(d.data.hospital);}setSL(false);},[]);
  const ldOrders=useCallback(async(f?:any)=>{setOL(true);const p=new URLSearchParams();const fl=f||oFilter;if(fl.status)p.set("status",fl.status);if(fl.priority)p.set("priority",fl.priority);if(fl.search)p.set("search",fl.search);if(fl.date)p.set("date",fl.date);p.set("limit","50");const d=await fetch(withSub(`/api/pathology/orders?${p}`),{credentials:"include"}).then(r=>r.json());if(d.success){setOrders(d.data.data||[]);setOTotal(d.data.total||0);}setOL(false);},[oFilter]);
  const ldTests=useCallback(async()=>{setTL(true);const d=await fetch(withSub("/api/pathology/tests"),{credentials:"include"}).then(r=>r.json());if(d.success)setTests(d.data||[]);setTL(false);},[]);
  const ldPanels=useCallback(async()=>{setPL(true);const d=await fetch(withSub("/api/pathology/panels"),{credentials:"include"}).then(r=>r.json());if(d.success)setPanels(d.data||[]);setPL(false);},[]);
  const ldSamples=useCallback(async()=>{setSmL(true);const d=await fetch(withSub("/api/pathology/samples?limit=50"),{credentials:"include"}).then(r=>r.json());if(d.success)setSamples(d.data||[]);setSmL(false);},[]);
  const ldReports=useCallback(async()=>{setRL(true);const d=await fetch(withSub("/api/pathology/reports?limit=30"),{credentials:"include"}).then(r=>r.json());if(d.success)setReports(d.data||[]);setRL(false);},[]);
  ...
  const saveRes=async()=>{if(!resOrder)return;setResSav(true);setResMsg("");const res=await fetch(withSub(`/api/pathology/orders/${resOrder.id}`),{method:"PUT",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({status:"RESULT_ENTERED",pathologistRemarks:resExtra.pathRemarks,interpretation:resExtra.interpretation,impression:resExtra.impression,verifiedBy:resExtra.verifiedBy,deliveryMode:resExtra.deliveryMode,items:resItems.map((i:any)=>({id:i.id,result:i.result,isAbnormal:i.isAbnormal,isCritical:i.isCritical,notes:i.notes,enteredBy:i.enteredBy,method:i.method,status:"RESULT_ENTERED"}))}))}).then((r:any)=>r.json());if(res.success){try{const tmpl:any=JSON.parse(localStorage.getItem("lab_res_templates")||"{}" );resItems.forEach(item=>{if(item.result)tmpl[item.testName]={unit:item.unit,normalRange:item.normalRange,enteredBy:item.enteredBy};});localStorage.setItem("lab_res_templates",JSON.stringify(tmpl));}catch{}setResOrder(null);await ldOrders();}else setResMsg(res.message||"Failed");setResSav(false);};
  const generateAiResults=async()=>{
    ...
    try{
      ...
      const res=await fetch(withSub("/api/pathology/ai-results"),{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody(body))}).then(r=>r.json());
  ...
  const doRep=async(orderId:string,action:string,dm?:string)=>{setRActL(true);const res=await fetch(withSub("/api/pathology/reports"),{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({orderId,action,deliveryMethod:dm}))}).then(r=>r.json());if(res.success){setVRep(null);await Promise.all([ldReports(),ldOrders()]);}else alert(res.message||"Failed");setRActL(false);};
  const deleteReport=async()=>{if(!delRep)return;setDelRepL(true);try{const r=await fetch(withSub(`/api/pathology/reports?reportId=${delRep.id}`),{method:"DELETE",credentials:"include"}).then(r=>r.json());if(r.success){setDelRep(null);await Promise.all([ldReports(),ldOrders()]);}else alert(r.message||"Failed to delete");}catch(e:any){alert(e.message||"Error");}finally{setDelRepL(false);};};
  const saveEditReport=async()=>{if(!editRep)return;setEditRepSaving(true);setEditRepMsg("");try{const r=await fetch(withSub("/api/pathology/reports"),{method:"PATCH",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({reportId:editRep.id,verifiedBy:editRepForm.verifiedBy,notes:editRepForm.notes,items:editRepItems}))}).then(r=>r.json());if(r.success){setEditRep(null);await ldReports();}else setEditRepMsg(r.message||"Failed");}catch(e:any){setEditRepMsg(e.message||"Error");}finally{setEditRepSaving(false);};};
  const sendRepEmail=async(r:any)=>{if(!r.order?.patient?.email){alert("No email on file for this patient");return;}if(!confirm(`Send report to ${r.order.patient.email}?`))return;setRActL(true);const res=await fetch(withSub("/api/pathology/reports"),{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({orderId:r.orderId,action:"deliver",deliveryMethod:"EMAIL"}))}).then(x=>x.json());if(res.success)await Promise.all([ldReports(),ldOrders()]);else alert(res.message||"Failed");setRActL(false);};
  const genRep=async(oid:string)=>{setRActL(true);const res=await fetch(withSub("/api/pathology/reports"),{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({orderId:oid,action:"generate"}))}).then(r=>r.json());if(res.success){await Promise.all([ldOrders(),ldReports()]);const ord=orders.find((o:any)=>o.id===oid);if(ord?.patient?.email){const send=confirm(`✅ Report generated!\n\nSend to patient email?\n${ord.patient.email}`);if(send)await doRep(oid,"deliver","EMAIL");}}else alert(res.message||"All results must be entered first.");setRActL(false);};
  const saveTest=async()=>{if(!tForm.name||!tForm.code){setTMsg("Name and code required");return;}setTSav(true);setTMsg("");const url=editT?`/api/pathology/tests/${editT.id}`:"/api/pathology/tests";const res=await fetch(withSub(url),{method:editT?"PUT":"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody(tForm))}).then(r=>r.json());if(res.success){setShowTF(false);setEditT(null);setTForm(BT);await ldTests();}else setTMsg(res.message||"Failed");setTSav(false);};
  const savePnl=async()=>{if(!pForm.name||!pForm.code){setPMsg("Name and code required");return;}setPSav(true);setPMsg("");const url=editP?`/api/pathology/panels/${editP.id}`:"/api/pathology/panels";const res=await fetch(withSub(url),{method:editP?"PUT":"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody(pForm))}).then(r=>r.json());if(res.success){setShowPF(false);setEditP(null);setPForm(BP);await ldPanels();}else setPMsg(res.message||"Failed");setPSav(false);};
  const delItem=async()=>{if(!delT)return;setDelL(true);await fetch(withSub(`/api/pathology/${delT.type==="test"?"tests":"panels"}/${delT.item.id}`),{method:"DELETE",credentials:"include"});setDelT(null);setDelL(false);delT.type==="test"?await ldTests():await ldPanels();};
          ...
          <div className="card"><table className="pt"><thead><tr><th>Code</th><th>Name</th><th>Category</th><th>Specimen</th><th>Normal Range</th><th>Price</th><th>TAT</th><th>Active</th><th></th></tr></thead><tbody>
            {tL?<tr><td colSpan={9} className="empty"><Loader2 size={13} className="spin" style={{verticalAlign:"middle"}}/></td></tr>:fTests.length===0?<tr><td colSpan={9} className="empty">No tests — click "Add Test"</td></tr>:fTests.map((t:any)=>(
              <tr key={t.id}>
                ...
                <td><button onClick={()=>fetch(withSub(`/api/pathology/tests/${t.id}`),{method:"PUT",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(withSubBody({isActive:!t.isActive}))}).then(()=>ldTests())} style={{background:"none",border:"none",cursor:"pointer",color:t.isActive?A:"#cbd5e1",padding:0}}>{t.isActive?<ToggleRight size={16}/>:<ToggleLeft size={16}/>}</button></td>
                <td><div style={{display:"flex",gap:2}}>
