...
export const PUT = withApiRoute("expense.id.put", async (req: NextRequest, { params }: { params: { id: string } }) => {
...
export const DELETE = withApiRoute("expense.id.delete", async (req: NextRequest, { params }: { params: { id: string } }) => {
  const auth = await requireRole(req, ALLOWED);
  if (auth.error) return auth.error;
  try {
