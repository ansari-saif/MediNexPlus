...
export const PUT = withApiRoute("pathology.panels.id.put", async (req: NextRequest, { params }: { params: { id: string } }) => {
...
export const DELETE = withApiRoute("pathology.panels.id.delete", async (req: NextRequest, { params }: { params: { id: string } }) => {
  const { user, error } = await authMiddleware(req);
  if (error) return error;
  if (!["SUB_DEPT_HEAD", "HOSPITAL_ADMIN"].includes(user!.role)) return errorResponse("Forbidden", 403);
